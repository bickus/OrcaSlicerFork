...
Option ConfigOptionsGroup::get_option(const std::string& opt_key, int opt_index /*= -1*/)
{
	if (!m_config->has(opt_key)) {
        if (const ConfigDef *def = m_config->def(); def != nullptr && def->get(opt_key) != nullptr) {
            // Instantiate the option with its default value so downstream accessors have something to read.
            const_cast<DynamicPrintConfig*>(m_config)->option(opt_key, true);
        } else {
