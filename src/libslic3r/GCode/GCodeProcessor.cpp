...
bool GCodeProcessor::process_ideamaker_tags(const std::string_view comment)
{
    ...
    if (pos != comment.npos) {
        ...
        if (type == "RAFT")
            ...
            set_extrusion_role(erInternalBridgeInfill);
        else if (type == "EXTRA BRIDGE")
            set_extrusion_role(erExtraBridgeInfill);
        else if (type == "EXTRA INTERNAL BRIDGE")
            set_extrusion_role(erExtraInternalBridgeInfill);
        else if (type == "SUPPORT")
            set_extrusion_role(erSupportMaterial);
        else {
            set_extrusion_role(erNone);
...
void GCodeProcessor::process_G1(const GCodeReader::GCodeLine& line, const std::optional<unsigned int>& remaining_internal_g1_lines)
{
    ...
    if (type == EMoveType::Extrude) {
        ...
        else if (m_extrusion_role == erBridgeInfill || m_extrusion_role == erInternalBridgeInfill || m_extrusion_role == erExtraBridgeInfill || m_extrusion_role == erExtraInternalBridgeInfill || m_extrusion_role == erNone)
