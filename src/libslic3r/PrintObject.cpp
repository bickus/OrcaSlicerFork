    ...
    if (this->set_started(posEstimateCurledExtrusions)) {
        if ( std::any_of(this->print()->m_print_regions.begin(), this->print()->m_print_regions.end(),
            ...
            SupportSpotsGenerator::Params params{this->print()->m_config.filament_type.values,
                                                 float(this->print()->default_region_config().inner_wall_acceleration.getFloat()),
    ...
    const ConfigOptionResolver &old_config, const ConfigOptionResolver &new_config, const std::vector<t_config_option_key> &opt_keys)
{
    ...
    for (const t_config_option_key &opt_key : opt_keys) {
        if (   opt_key == "brim_width"
            ...
            || opt_key == "initial_layer_min_bead_width") {
            steps.emplace_back(posSlice);
        } else if (
...
void PrintObject::detect_surfaces_type()
{
    ...
    for (size_t region_id = 0; region_id < this->num_printing_regions(); ++ region_id) {
        ...
        // NOTE: Extra external bridge layer creation (stInternalAfterExternalBridge) has been moved to
        // bridge_over_infill() to run AFTER process_external_surfaces() detects bridge angles.
...
void PrintObject::bridge_over_infill()
{
    ...
    // === This runs AFTER process_external_surfaces() so that stBottomBridge surfaces have their bridge_angle set. ====================
    // ======================================================================================================================================
    if ( this->m_config.enable_extra_bridge_layer == eblApplyToAll || this->m_config.enable_extra_bridge_layer == eblExternalBridgeOnly) {
        // Process layers in parallel up to second-to-last
        tbb::parallel_for( tbb::blocked_range<size_t>(0, this->layers().size() - 1), [this](const tbb::blocked_range<size_t>& r) {
            for (size_t lidx = r.begin(); lidx < r.end(); ++lidx)
            {
                ...
                // (A) Gather external bridging surfaces in the current layer
                ExPolygons bridging_current_layer;
                double bridging_angle_current = 0.0;
                ...
                for (LayerRegion *region : layer->regions()) {
                    for (const Surface &surf : region->fill_surfaces.surfaces) {
                        if (surf.surface_type == stBottomBridge) {
                            bridging_current_layer.push_back(surf.expolygon);
                            bridging_angle_current = surf.bridge_angle; // Store the last bridging angle
                            found_any_bridge = true;
                        }
                    }
                }
                ...
                // (C) If there is a next layer, identify overlapping stInternal & stInternalSolid areas and convert the overlap to stInternalAfterExternalBridge
                if (lidx + 1 < this->layers().size()) {
                    Layer* next_layer = this->get_layer(lidx + 1);
                    ...
                    for (LayerRegion *next_region : next_layer->regions()) {
                        ...
                        // 2b) For every collected next internal surface
                        for (const Surface *s : next_internals) {
                            // Intersect it with the current layer bridging polygons
                            ExPolygons overlap = intersection_ex( s->expolygon, bridging_union, ApplySafetyOffset::Yes );
                            ...
                            // Overlapping portion found -> this will become the second external bridge
                            if (!overlap.empty()) {
                                // Create second bridge surface
                                Surface tmp{*s, {}};
                                tmp.surface_type = stInternalAfterExternalBridge;
                                tmp.bridge_angle = bridging_angle_second;
    ...
    // ===========================================================================================
    // === ORCA: End of second external bridging pass ============================================
    // ===========================================================================================
...
// Overload that merges plate layer config ranges with object layer config ranges
...
bool PrintObject::update_layer_height_profile(const ModelObject &model_object, const SlicingParameters &slicing_parameters, std::vector<coordf_t> &layer_height_profile, const t_layer_config_ranges &plate_layer_ranges)
{
    ...
    if (layer_height_profile.empty() || layer_height_profile[1] != slicing_parameters.first_object_layer_height) {
        ...
        for (const auto& plate_range : plate_layer_ranges) {
            ...
            coordf_t current_lo = plate_lo;

            // Sort object ranges by their start position for this splitting algorithm
            std::vector<std::pair<coordf_t, coordf_t>> overlapping_obj_ranges;
            for (const auto& obj_range : object_ranges) {
            ...
            std::sort(overlapping_obj_ranges.begin(), overlapping_obj_ranges.end());
