...
## Assumptions & Notes
- Keeping the option as `ConfigOptionFloats` (instead of a scalar) lets the existing machine-limit normalization logic validate each column against the configured `[min,max]` range, avoiding the `[0,0]` CLI failures you see when a new scalar key is fed through that path.
